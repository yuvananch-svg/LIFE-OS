...
create or replace function public.assert_balanced_finance_transaction() returns trigger language plpgsql as $$
declare tx uuid; total numeric;
begin
  if tg_op = 'DELETE' or tg_op = 'UPDATE' then
    tx := old.transaction_id;
    select coalesce(sum(amount),0) into total from public.finance_entries where transaction_id=tx;
